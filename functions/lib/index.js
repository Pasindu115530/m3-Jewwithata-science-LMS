...
const zoomService_1 = require("./zoom/zoomService");
// ─── createZoomMeeting Cloud Function ──────────────────────────────────────────
exports.createZoomMeeting = (0, https_1.onCall)(async (request) => {
    try {
        if (!request.auth) {
            throw new https_1.HttpsError("unauthenticated", "Authentication required to create a Zoom meeting.");
        }
        const role = request.auth.token.role;
        if (role !== "teacher" && role !== "admin") {
...
exports.deleteZoomMeeting = (0, https_1.onCall)(async (request) => {
    try {
        if (!request.auth) {
            throw new https_1.HttpsError("unauthenticated", "Authentication required.");
        }
...
exports.processAttendance = (0, https_1.onCall)(async (request) => {
    try {
        ...
        if (err instanceof https_1.HttpsError)
            throw err;
        throw new https_1.HttpsError("internal", err.message || "Failed to process attendance.");
    }
});
...
exports.assignFreePhysicalCard = (0, https_1.onCall)(async (request) => {
    try {
        if (!request.auth)
            throw new https_1.HttpsError("unauthenticated", "Authentication required.");
        const role = request.auth.token.role;
        ...
        const { studentUid, smartCardNumber } = request.data;
        if (!studentUid || !smartCardNumber) {
            throw new https_1.HttpsError("invalid-argument", "Student ID and Smart Card Number are required.");
        }
        ...
        if (err instanceof https_1.HttpsError)
            throw err;
        throw new https_1.HttpsError("internal", err.message || "Failed to assign free physical card.");
    }
});
// ─── toggleTemporaryCourseAccess ─────────────────────────────────────────────
exports.toggleTemporaryCourseAccess = (0, https_1.onCall)(async (request) => {
    try {
        if (!request.auth)
            throw new https_1.HttpsError("unauthenticated", "Authentication required.");
        const role = request.auth.token.role;
        ...
        const { studentUid, grant, notes } = request.data;
        if (!studentUid || typeof grant !== "boolean") {
            throw new https_1.HttpsError("invalid-argument", "studentUid and grant flag are required.");
        }
        const userRef = db.collection("users").doc(studentUid);
        const userDoc = await userRef.get();
        if (!userDoc.exists)
        ...
        await userRef.update({
            ...
            temporaryAccessUpdatedAt: firestore_1.FieldValue.serverTimestamp(),
        });
        return {
            success: true,
            grant,
        ...
        if (err instanceof https_1.HttpsError)
            throw err;
        throw new https_1.HttpsError("internal", err.message || "Failed to update temporary access.");
    }
});
